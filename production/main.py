...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="Main Script Run") as main_run:
        ...
        with mlflow.start_run(run_name="Data Cleaning", nested=True):
            result1 = subprocess.run([sys.executable, '/Documents/asiignment 5.1/housing-price-prediction-main/production'], capture_output=True, text=True)
        ...
        with mlflow.start_run(run_name="Feature Engineering", nested=True):
            result1 = subprocess.run([sys.executable, '/Documents/asiignment 5.1/housing-price-prediction-main/production/feature_engineering.py'], capture_output=True, text=True)
            transform_features(context,params = {"outliers": {"method": "mean","drop": False }})
            
        with mlflow.start_run(run_name="Training", nested=True):
            result1 =  subprocess.run([sys.executable, 'Documents/asiignment 5.1/housing-price-prediction-main/production/training.py'], capture_output=True, text=True)
            train_model(context,{})
            # mlflow.log_param("param_name", res)
        with mlflow.start_run(run_name="Scoring", nested=True):
            result1 = subprocess.run([sys.executable, 'Documents/asiignment 5.1/housing-price-prediction-main/production/scoring.py'], capture_output=True, text=True)
            score_model(context,{})
